...
class VectorCache:
    ...
    def save_catalog_vectors(
        self, catalog_hash: str, catalog: List[str], embeddings: np.ndarray, faiss_index: faiss.Index
    ):
    ...
    def _cleanup_old_catalogs(self):
        ...
        sorted_catalogs = sorted(self.catalogs.items(), key=lambda x: x[1]["last_used"])
